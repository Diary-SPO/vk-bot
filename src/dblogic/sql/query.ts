...
class QUERY {
    ...
    columns(columns: string[]): QUERY {
        if (this.COLUMNS !== '*')
             this.COLUMNS = [this.COLUMNS, ...columns].join(', ');
        else this.COLUMNS = columns.join(', ');
        return this;
    }
...
class WHERE {
    ...
    ANDIF(condition: string | WHERE): WHERE {
        ...
        else this.CONDITIONS += `AND  (${condition.get()})`
        return this;
    }
    ORIF(condition: string | WHERE): WHERE {
        if (typeof(condition) == 'string')
             this.CONDITIONS += ` OR ${condition}`;
        else this.CONDITIONS += ` OR (${condition.get()})`
        return this;
    }
    get() {
...
export interface Insert_values {[key: string]: string | number}